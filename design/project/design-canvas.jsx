...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useLayoutEffect(() => {
    ...
    // Frame the first section at a readable zoom, top-centered. Used on a
    // fresh load and as a recovery when a restored/host camera leaves all
    // content off-screen (otherwise the canvas would open blank).
    const frameFirst = () => {
      const world = worldRef.current, vp = vpRef.current;
      const firstSec = world && world.querySelector('[data-dc-section]');
      if (!firstSec || !vp) return;
      const vpW = vp.clientWidth, vpH = vp.clientHeight;
      // The section block stretches to the full (max-content) world width, so
      // measure its inner artboard row instead for a sensible fit width.
      const row = firstSec.querySelector(':scope > div:last-child') || firstSec;
      const secW = row.offsetWidth || firstSec.offsetWidth || 1;
      const secLeft = row.offsetLeft || 0, secTop = firstSec.offsetTop;
      const margin = 56;
      let scale = Math.min(1, (vpW - margin * 2) / secW);
      scale = Math.max(minScale, Math.min(maxScale, scale));
      tf.current.scale = scale;
      tf.current.x = Math.max(margin, (vpW - secW * scale) / 2) - secLeft * scale;
      tf.current.y = Math.min(margin, vpH * 0.08) - secTop * scale + 24 * scale;
      apply();
    };
    // True when the first section's box overlaps the viewport at all.
    const contentVisible = () => {
      const vp = vpRef.current;
      const firstSec = vp && vp.querySelector('[data-dc-section]');
      if (!firstSec) return false;
      const r = firstSec.getBoundingClientRect();
      const vw = vp.clientWidth, vh = vp.clientHeight;
      return r.right > 48 && r.left < vw - 48 && r.bottom > 48 && r.top < vh - 48;
    };
    // Children mount a tick (sometimes several) after this effect —
    // DesignCanvas gates them on an async sidecar read. Poll on an interval
    // until the first section exists, then decide whether to keep the
    // restored camera or reframe. Keep re-checking briefly afterward so a
    // late host __dc_set_zoom or layout shift that pushes everything
    // off-screen still gets recovered instead of opening blank.
    let elapsed = 0, framedOnce = false;
    const poll = setInterval(() => {
      elapsed += 120;
      const firstSec = vpRef.current && vpRef.current.querySelector('[data-dc-section]');
      if (!firstSec) { if (elapsed > 6000) clearInterval(poll); return; }
      if (!framedOnce) {
        if (!restored || !contentVisible()) frameFirst();
        framedOnce = true;
      } else if (!contentVisible()) {
        frameFirst();
      }
      if (elapsed > 2500) clearInterval(poll);
    }, 120);

    // Flush on pagehide and unmount so a reload within the 200ms debounce
    // window doesn't drop the last pan/zoom.
    window.addEventListener('pagehide', flush);
    return () => { clearInterval(poll); window.removeEventListener('pagehide', flush); flush(); };
  }, []);
