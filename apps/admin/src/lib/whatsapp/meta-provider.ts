...
export class MetaWhatsAppProvider implements WhatsAppProvider {
  ...
  /**
   * Create a one-time-passcode (OTP) template under the AUTHENTICATION
   * category. Body text is provided by Meta — only the OTP code button shape
   * and optional security/expiration extras are configurable.
   */
  async createAuthTemplate(
    businessId: string,
    name: string,
    language: string,
    options: {
      ...
      const components: Record<string, unknown>[] = [
        {
          ...
          add_security_recommendation:
            options.addSecurityRecommendation ?? true,
        },
      ];

      if (options.codeExpirationMinutes) {
        components.push({
          type: "FOOTER",
          code_expiration_minutes: options.codeExpirationMinutes,
        });
      }
        ...
        buttons: [
          {
            type: "OTP",
            otp_type: "COPY_CODE",
            text: options.copyButtonText ?? "Copiar código",
          },
        ],
      });
