...
export async function POST(
  ...
  const isAuthTemplate = plantilla.tipo === "AUTENTICACION_OTP";

  async function createTemplateForPlantilla(targetName: string) {
    if (isAuthTemplate) {
      return provider!.createAuthTemplate(
        businessId!,
        targetName,
  ...
  if (!plantilla.whatsappTemplateName) {
    ...
    const result = await createTemplateForPlantilla(templateName);
  ...
  const result = await createTemplateForPlantilla(pendingName);
